...
(function(){
  ...
  // ---- RECIPES: read from the hidden CMS list #hb-rcp-src on the home page ----
  // Adding a recipe in the CMS is enough; nothing here needs regenerating.
  // Food -> recipe matching, shared by the widget build (copied verbatim) and this test.
  var ALIAS={'beets':['beetroot'],'soy-tofu':['tofu'],'grass-fed-beef':['steak','beef mince'],'extra-virgin-olive-oil':['olive oil'],
   'raw-honey':['honey'],'ghee-grass-fed-butter':['ghee','butter'],'parmesan-hard-cheese':['parmesan'],'cacao-nibs':['cacao'],
   'citrus':['lemon'],'beans':['black beans','white beans'],'shrimp':['prawns'],'onions':['onion','spring onions','shallots'],
   'liver':['chicken livers'],'sesame-seeds':['sesame seeds','tahini'],'bananas':['banana'],'almonds':['almond butter']};
  function foodTerms(slug){
    if(ALIAS[slug]) return ALIAS[slug];
    var t=slug.replace(/-/g,' '); return [t, t.replace(/s$/,'')];
  }
  
  var RCP_CACHE=null;
  function readRecipes(){
    if(RCP_CACHE) return RCP_CACHE;
    var out=[], seen={};
    var src=D.getElementById('hb-rcp-src');
    if(src){
      [].forEach.call(src.querySelectorAll('[data-r]'),function(it){
        ...
        out.push({ s:s, n:n, x:g('intro'), m:g('meal'),
          t:parseInt(g('time'),10)||0, pr:parseInt(g('protein'),10)||0, k:parseInt(g('kcal'),10)||0,
          d:g('diet').toLowerCase(),
          tg:g('tags').toLowerCase().split(',').map(function(x){return x.trim();}).filter(Boolean),
          p:p });
      });
    }
  ...
  function dietLabel(d){
    ...
    if(/gluten-free/.test(d)) return 'Gluten-free';
    return '';
  }
  ...
  function wire(root){
    ...
    var ordRcp=seededOrder(RECIPES.length, DAY+71);
    ...
    // ---- COOK IT: a recipe that uses today's food. Space is always reserved
    // (visibility, not display) so the card never changes height as it rotates.
    function currentRecipe(){ return RECIPES.length ? RECIPES[ordRcp[st.rcp%RECIPES.length]] : null; }
    function paintCook(f){
      var a=foodCard.querySelector('[data-cook]'); if(!a) return;
      var pick=null;
      if(f && f.e==='Supports' && RECIPES.length){
        var terms=foodTerms(f.s), cur=currentRecipe(), alt=null;
        for(var i=0;i<ordRcp.length;i++){
          var r=RECIPES[ordRcp[i]];
          if(!r.tg.some(function(t){ return terms.indexOf(t)>=0; })) continue;
          if(cur && r.s===cur.s){ if(!alt) alt=r; continue; }
          pick=r; break;
        }
        if(!pick) pick=alt;
      }
      if(pick){
        a.setAttribute('href','/recipes/'+pick.s);
        a.querySelector('b').textContent=pick.n;
        a.classList.remove('off'); a.removeAttribute('aria-hidden'); a.removeAttribute('tabindex');
      } else {
        a.classList.add('off'); a.setAttribute('aria-hidden','true'); a.setAttribute('tabindex','-1');
        a.setAttribute('href','/recipes');
      }
    }

    // ---- RECIPE: same timing and cross-fade as the food card ----
    function paintRecipe(first,soft){
      if(!rcpCard) return;
      if(!RECIPES.length){ rcpCard.style.display='none'; return; }
      var r=currentRecipe();
      if(!first){ rcpCard.classList.add('swapping'); if(soft) rcpCard.classList.add('soft'); }
      function apply(){
        var bg=rcpCard.querySelector('.bg'), fb=rcpCard.querySelector('.fallback');
        if(fb) fb.style.display='none';
        if(bg){ bg.style.display=''; bg.style.backgroundImage=''; }
        preload(r.p,function(ok){
          if(ok){ if(bg){ bg.style.backgroundImage="url('"+r.p+"')"; bg.style.display=''; } if(fb) fb.style.display='none'; }
          else { if(bg) bg.style.display='none'; if(fb) fb.style.display='flex'; }
        });
        var chip=rcpCard.querySelector('.hb-t-chip');
        chip.textContent=[r.m, r.t?r.t+' min':''].filter(Boolean).join(' \u00B7 ');
        chip.style.display=chip.textContent?'':'none';
        rcpCard.querySelector('.hb-t-name').textContent=r.n;
        rcpCard.querySelector('.hb-t-text').textContent=r.x;
        var tags=[];
        if(r.pr) tags.push(r.pr+'g protein');
        if(r.k) tags.push(r.k+' kcal');
        var dl=dietLabel(r.d); if(dl) tags.push(dl);
        rcpCard.querySelector('.hb-t-meta').innerHTML=tags.map(function(t){return '<span class="hb-t-tag">'+esc(t)+'</span>';}).join('');
        var cta=rcpCard.querySelector('.hb-t-cta'); if(cta) cta.setAttribute('href','/recipes/'+r.s);
        var ovl=rcpCard.querySelector('.hb-t-cardlink'); if(ovl) ovl.setAttribute('href','/recipes/'+r.s);
        rcpCard.classList.remove('skeleton','swapping');
        if(soft) setTimeout(function(){ rcpCard.classList.remove('soft'); },600);
        say(r.n);
      }
      setTimeout(apply, first?0:(soft?560:160));
    }
    ...
    // initial paint
    paintRecipe(true); paintFood(true); paintIns(true); paintList('men',true); paintList('women',true); paintDots();

    // ---- gentle autoplay: cycles slowly, pauses on hover, stops for good on manual use ----
    var AUTO_MS=4000, AUTO_DELAY=4000, AUTO_RESUME=15000;
    var reduced=false;
    ...
    function autoTick(){
      ...
      if(RECIPES.length&&rcpCard){ st.rcp=(st.rcp+1)%RECIPES.length; paintRecipe(false,true); }
    }
    ...
    function autoDefer(){
      ...
      autoPaused=true;
      if(deferT) clearTimeout(deferT);
      deferT=setTimeout(function(){ autoPaused=false; },AUTO_RESUME);
    }
    if(!autoStopped){
    ...
    // ---- interactions ----
    [].forEach.call(root.querySelectorAll('[data-nav]'),function(btn){
      btn.addEventListener('click',function(ev){
        ev.preventDefault(); ev.stopPropagation();
        autoDefer();
        var k=btn.dataset.nav, d=parseInt(btn.dataset.dir,10)||1;
        if(k==='food'&&FOODS.length){ st.food=((st.food+d)%FOODS.length+FOODS.length)%FOODS.length; paintFood(false); track(d>0?'nav_next':'nav_prev','food'); }
        if(k==='insight'&&insight.length){ st.ins=((st.ins+d)%insight.length+insight.length)%insight.length; paintIns(false); track(d>0?'nav_next':'nav_prev','insight'); }
        if(k==='recipe'&&RECIPES.length){ st.rcp=((st.rcp+d)%RECIPES.length+RECIPES.length)%RECIPES.length; paintRecipe(false); track(d>0?'nav_next':'nav_prev','recipe'); }
      });
    });
    [].forEach.call(root.querySelectorAll('[data-shuffle]'),function(btn){
      btn.addEventListener('click',function(){
        ...
        if(k==='recipe'&&RECIPES.length){ st.rcp=(st.rcp+1)%RECIPES.length; paintRecipe(false); track('shuffle','recipe'); }
      });
    });
    var cookA=foodCard&&foodCard.querySelector('[data-cook]');
    if(cookA) cookA.addEventListener('click',function(){ track('link_click','cook:'+cookA.getAttribute('href')); });
    var rcpCta=rcpCard&&rcpCard.querySelector('.hb-t-cta');
    if(rcpCta) rcpCta.addEventListener('click',function(){ track('link_click','recipe:'+rcpCta.getAttribute('href')); });
