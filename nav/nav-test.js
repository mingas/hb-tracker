...
setTimeout(() => {
  ...
  t('Nutrition 3 items', nut.querySelectorAll('.nbm-dd-link').length === 3);
  t('Nutrition order Supplements/Foods/Recipes',
    Array.from(nut.querySelectorAll('.nbm-dd-link')).map(a=>a.textContent).join('|') === 'Supplements|Foods|Recipes',
    Array.from(nut.querySelectorAll('.nbm-dd-link')).map(a=>a.textContent).join('|'));
  t('Recipes href correct',
    Array.from(nut.querySelectorAll('.nbm-dd-link')).some(a=>a.getAttribute('href')==='/recipes'));
